Feature: multiple header management approaches that demonstrate how after
  ...
  refer to 'call-updates-config.feature' and 'common.feature'
    ...
    * configure headers = read('../headers.js')
    ...
    And param url = urlBase
    When method get
    Then status 200

  Scenario: configure json
    * configure headers = { Authorization: '#(token + time + urlBase)' }
    Given path 'headers', token
    And param url = urlBase
    When method get
    Then status 200
  ...
  Scenario: set header
    * header Authorization = token + time + urlBase
    Given path 'headers', token
    And param url = urlBase
    When method get
    Then status 200
  ...
  Scenario: multi-value headers
    * header Authorization = 'dummy', token + time + urlBase
    Given path 'headers', token
    And param url = urlBase
    When method get
    Then status 200
  ...
  Scenario: set headers using json
    * headers { Authorization: '#(token + time + urlBase)' }
    Given path 'headers', token
    And param url = urlBase
    When method get
    Then status 200
  ...
  Scenario: set multi-value headers using json
    * headers { Authorization: ['dummy', '#(token + time + urlBase)'] }
    Given path 'headers', token
    And param url = urlBase
    When method get
    Then status 200
  ...
  Scenario: set multi-value headers using function call
    ...
    * header Authorization = call fun { first: 'dummy', second: '#(token + time + urlBase)' }
    Given path 'headers', token
    And param url = urlBase
    When method get
    Then status 200